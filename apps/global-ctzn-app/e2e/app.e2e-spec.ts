...
describe('Starting tests for global-ctzn-app', function() {
  ...
  it('website title should be global-ctzn-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('global-ctzn-app');
    })
  });

  it('network-name should be global-ctzn-network@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('global-ctzn-network@0.0.1.bna');
    });
  });

  it('navbar-brand should be global-ctzn-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('global-ctzn-app');
    });
  });
    ...
    it('GovOrg table should have 5 columns',() => {
      page.navigateTo('/GovOrg');
      element.all(by.css('.thead-cols th')).then(function(arr) {
