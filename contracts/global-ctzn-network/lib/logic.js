...
function createProjectPledge(txParams) {
  ...
  return getAssetRegistry(NS + '.ProjectPledge').then(function (registry) {
    pledge = factory.newResource(NS, 'ProjectPledge', Math.random().toString(36).substring(3));
...
function transferFunds(txParams) {
  ...
  for(var i = 0; i < txParams.pledgeId.funds.length; i++) {
    if(txParams.pledgeId.funds[i].govOrgId === txParams.govOrgId) {
      txParams.pledgeId.funds[i].lastFundsReceived = new Date();
      var daysToAdd = 0;
      ...
      txParams.pledgeId.funds[i].nextFundingDueDate.setDate(txParams.pledgeId.funds[i].nextFundingDueDate.getDate() + daysToAdd);
